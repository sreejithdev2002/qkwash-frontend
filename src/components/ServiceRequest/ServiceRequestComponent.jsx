...
function ServiceRequestComponent() {
  ...
  return (
    <div className="overflow-auto h-[80vh] pr-5" id="scrollbar barlowFont">
      {tableData.map((data) => (
        <div className="font-medium bg-[#D9D9D9] hover:bg-[#d1d1d1] flex justify-between px-2 lg:px-10 py-2 lg:py-5 my-2 lg:my-5 text-center rounded-lg lg:rounded-2xl cursor-pointer transition-all duration-300">
          <div>
            <h1 className="text-[10px] lg:text-2xl text-[#464255] text-start">
              {data.hubname}
            </h1>
            <p className="text-[8px] lg:text-xl text-start text-[#A3A3A3]">#{data.hubid}</p>
          </div>
          <div className="flex flex-col text-[#464255] justify-between text-[8px] lg:text-lg">
            <p>{data.devicetype}</p>
          </div>
          <div className="flex flex-col text-[#464255] justify-between text-[8px] lg:text-lg">
            <p>{data.Serviceid}</p>
